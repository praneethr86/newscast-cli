...
function getContent(modifier, value, cb) {
  ...
  if (modifier == 'eds') {
    if (value == 'hindu') {
      ...
      newssrc.edsHBL(EDITORIAL_HBL_HOST, cb);
    } else if (value == 'guardian') {
      newssrc.edsGuardian(EDITORIAL_GUARDIAN_HOST, cb);
    } else if (value == 'economist') {
      newssrc.edsEconomist(ECONOMIST_LATEST_HOST, cb);
    }
  ...
  //   }
  // }
  else if (modifier === 'projsyn') {
    if (value === 'all') {
      newssrc.projSyndicate(PROJECT_SYNDICATE_HOST, cb);
    }
  } else if (modifier === 'hbr') {
    if (value === 'latest') {
      newssrc.hbrLatest(HBR_LATEST_HOST, cb);
    }
  }
}
