...
function getContent(modifier, value, cb) {
  if (modifier === "eds") {
    if (value === "hindu") {
      ...
      editorials.edsBizStandard(hosts.EDITORIAL_BUSINESS_STANDARD, cb);
    } else if (value === "et") {
    ...
    if (value === "economist") {
      ...
      magazines.magzWEF(hosts.WEF_AGENDA_HOST, cb);
    ...
    } else if (value === "longreads") {
      magazines.magzLongReads(hosts.LONGREADS_LATEST_HOST, cb);
    } else if (value === "outlook") {
      magazines.magzOutlook(hosts.OUTLOOK_LATEST_HOST, cb);
    } else if (value === "hinduexplains") {
      magazines.magzHinduExplains(hosts.THEHINDU_EXPLAINS_HOST, cb);
    }
    ...
    if (value === "sciam") {
      ...
      science.sciEmptyYourCup(hosts.SCIENCE_EMPTYYOURCUP_HOST, cb);
    }
    ...
    if (value === "techcrunch") {
      ...
      tech.techReadWrite(hosts.TECH_READWRITE_HOST, cb);
    } else if (value === "ken") {
      tech.techKen(hosts.TECH_THEKEN_HOST, cb);
    } else if (value === "bi") {
      tech.techBI(hosts.TECH_BI_HOST, cb);
    }
  }
}
